async function instantiate ({ connection, schemas, noRebuild }) {
  ...
  for (const schema of schemas) {
    this.storage[schema.name] = this.storage[schema.name] ?? [] // init empty coll
  }
  if (this.config.persistence.collections.length > 0) {
    const dir = `${getPluginDataDir('bajoDbMemory')}/data`
    fs.ensureDirSync(dir)
    // load
    for (const key of this.config.persistence.collections) {
      if (!this.storage[key]) this.fatal('Invalid collection for persistence: %s', key)
